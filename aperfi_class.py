...
class aperfi_class:
	...
	def find_rfi(self):
		...
		datacube = np.zeros([len(self.baselines_sort),self.vis.shape[1],self.vis.shape[0]/(len(self.baselines_sort))])
		baseline_counter = np.zeros((self.nant,self.nant),dtype=int)
		
		self.rms = np.zeros([datacube.shape[0],datacube.shape[1]])
		self.mean_array = np.zeros([datacube.shape[0],datacube.shape[1]])
		self.flag_lim_array= np.zeros([datacube.shape[0]])
	...
	def make_beam(self): 
		...
		fits = lib.miriad('fits')
   		fits.op = 'uvin'
        	fits.in_ = self.rfifile.rstrip('MS') + 'UVFITS'
        	fits.out = self.rfifile.rstrip('MS') + 'mir'
        	fits.go()
	...
	def write_freq_base(self) :
		...
		#write file
		hdulist.writeto(self.rfi_freq_base,self.rms,header)
		hdulist.close()
	...
	def plot_rfi_im(self):		
		...
		fig.savefig(self.rfi_freq_base_plot,format='png',overwrite=True)
